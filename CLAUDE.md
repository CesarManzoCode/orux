...
**Capa 10 pendiente (Git real remoto, decidida aparte por ser la parte difícil)**: clone de un repo existente + push/pull, que requieren credenciales git POR USUARIO sobre la web (guardar tokens de terceros = decisión de seguridad seria). El usuario eligió hacer primero capa 9 (delete+commit local) y decidir el modelo de credenciales después. NO hacer aún sin que el usuario lo pida: push/pull/remoto, branches/PRs, base de datos, escalado horizontal, multi-repo, multi-lenguaje, CRDT.
