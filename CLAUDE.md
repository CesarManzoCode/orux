...
- `tests/` — 103 tests. Capa 7: `autenticar(ws, user=, registrar=)` pasa la compuerta; `handshake()` = autenticar + init+welcome+ownership (los tests que leen `init` a mano hacen `await autenticar(c)` antes). Contratos intactos: broadcasts de update SIEMPRE con `path`, nunca eco al emisor. `test_*.py` puros: locks/presence/analysis/identity. Storage/tests usan `tmp_path`; users/ownership en memoria (None).
...
Capa 6 (análisis de impacto, Python) COMPLETA y validada end-to-end por el usuario (2 users). Capa 7 (IDENTIDAD REAL — password propio, login obligatorio) gradual: 1/3 núcleo puro ✅ · 2/3 auth sobre WebSocket + ownership por usuario persistido ✅ · 3/3 login/registro en el cliente ⏭ (el cliente web AÚN no funciona: el server está cerrado y el cliente todavía manda el token anónimo viejo; sin la UI de login del 3/3 no se entra). Server: `_autenticar` (register/login/session) ANTES de init/welcome/ownership; identidad = usuario real (Roster.asignar(usuario), color = hash del usuario); ownership por usuario y persistido (`Ownership(path)`), ya no se libera al desconectar; token de sesión firmado HMAC, secreto en `~/.laidea/secret`. `SyncServer(storage, users, ownership, secret)` — todo inyectado, None=memoria en tests. Después de capa 7: **integración con Git** (capa final). NO hacer aún: escalado horizontal, multi-repo, multi-lenguaje.
