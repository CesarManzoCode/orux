...
- `laidea/identity/` — capa 7 (1/3, núcleo puro): `passwords.py` (PBKDF2 stdlib, sal por usuario, verificación tiempo constante), `tokens.py` (token de sesión firmado HMAC, payload dict para futuro `exp`), `store.py` (`UserStore`: usuarios en JSON, DI por ruta como `DiskStorage`, usuario normalizado trim+minúsculas). Cero dependencias nuevas.
- `tests/` — 98 tests. Contratos intactos: `init` primer mensaje, broadcasts de update SIEMPRE con `path`, nunca eco de update al emisor. `handshake()` consume `init`+`welcome`+`ownership`; `recv_tipo()` filtra mensajes intercalados. Fixture `servidor` cede (SyncServer, port) para pre-sembrar workspace sin dueño. `test_locks.py`/`test_presence.py`/`test_analysis.py`/`test_identity.py` prueban piezas puras. Storage/tests usan `tmp_path`.
...
Capa 6 (análisis de impacto, Python) COMPLETA y validada end-to-end por el usuario (2 users). Capa 7 (IDENTIDAD REAL — password propio, login obligatorio, decisión del usuario; antes que Git/VPS porque es la viga de la que cuelga el "autor" y el ownership) en progreso gradual: 1/3 núcleo puro ✅ · 2/3 auth sobre WebSocket + ownership por usuario persistido ⏭ · 3/3 login/registro en el cliente ⏭. Después de capa 7: **integración con Git** (capa final). CRDT real solo si un perfilador/uso lo justifica — la tesis es prevenir, no fusionar. NO hacer aún: escalado horizontal, multi-repo, multi-lenguaje (sería adelantarse).
