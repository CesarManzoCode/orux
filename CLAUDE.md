...
- **Dev/deploy del cliente (capa 14, React).** Dev: `cd client && npm install && npm run dev` (Vite en :5173; `store.ts` apunta el WS a `ws://localhost:8765`, server suelto con `python -m laidea.server`). Deploy: `make up` — `Dockerfile.web` es multi-stage (Node compila `client/` → Caddy sirve `dist/`); el primer build necesita internet (corre en el VPS, no en este sandbox). Ya NO hay bind-mount de `web/` ni Live Server; el server estático que vigilaba la carpeta dejó de existir, así que la trampa de abajo es **histórica** (se conserva por la lección de fondo).
