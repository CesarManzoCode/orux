...
Este archivo define SOLO las FORMAS (las dataclasses). El paquete `protocol`
tiene otras dos mitades: `validation.py` (topes de tamaño y lectores de campo
defensivos) y `codec.py` (`encode`/`decode`). `protocol/__init__.py` re-exporta
todo, así que el resto del backend sigue haciendo `from ..protocol import ...`
sin enterarse del corte.
